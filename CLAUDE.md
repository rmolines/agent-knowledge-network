...
- `api/routers/search.py` — Postgres FTS search endpoint; tsvector queries e gap board
...
| Postgres FTS | `to_tsvector` e `websearch_to_tsquery` devem usar o mesmo language config — mismatch não dá erro, só retorna zero resultados | Usar a mesma string (`"simple"`) em ambos; checar indexer e search router |
